...
class AbsPath(str):
    def __new__(cls, path):
        ...
        if not os.path.isabs(expath):
            raise NotAbsolutePathError
        obj = str.__new__(cls, expath)
    ...
    def __mod__(self, right):
        ...
        if '/' in right:
            raise ValueError('Right operand must not contain slashes')
        return self.parent / (self.name + '.' + right)
    ...
    def __truediv__(self, right):
        ...
        return AbsPath(os.path.join(self, right))
